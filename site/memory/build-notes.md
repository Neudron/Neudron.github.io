# build notes — the old README, preserved

This is `README.md` as it stood until 2026-08-17, moved here whole rather
than deleted. It is 772 lines of build-log: how the sword swing was phased,
why the spawn animation was rebuilt, how the totem sound is synthesised, the
font provenance, and the Act I–II dev commands.

**It is out of date in one important way** — it documents the *pre-reorg*
file tree, when everything lived flat in `js/` (`js/main.js`, `js/scene.js`,
`js/data.js`). Those files are now under `js/page/`, `js/core/`, `js/data/`,
`js/game/` and `js/act4/`. Read paths here as historical.

Nothing else was thrown away: `README.md` is now a short front door for
someone who has never seen the project, and the reference material that was
buried in here lives in `memory/architecture.md`, `memory/decisions.md`,
`memory/design.md` and `memory/assets.md`.
